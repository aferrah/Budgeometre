...
def archiver_mois():
    ...
    stats_cat = defaultdict(lambda: {'nom': '', 'couleur': '#8b5cf6', 'depenses': 0, 'revenus': 0, 'nb_transactions': 0})
    trans_data = []
    for t in trans:
        ...
        if t.montant < 0: stats_cat[t.idCategorie]['depenses'] += float(-t.montant)
        else: stats_cat[t.idCategorie]['revenus'] += float(t.montant)
        trans_data.append({'titre': t.titre, 'montant': float(t.montant), 'date': t.dateTransaction.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    archive = ArchiveMensuelle(annee=annee, mois=mois, total_revenus=total_rev, total_depenses=total_dep,
        total_epargne=float(db.session.query(db.func.coalesce(db.func.sum(Objectif.epargne_actuelle), 0)).scalar()),
        solde_final=total_rev - total_dep, donnees_json=json.dumps({'transactions': trans_data, 'categories': dict(stats_cat),
            'stats': {'nb_transactions': len(trans), 'nb_depenses': sum(1 for t in trans if t.montant < 0), 'nb_revenus': sum(1 for t in trans if t.montant > 0)}}))
    db.session.add(archive)
    db.session.commit()
    flash(f"Archive {mois}/{annee} créée avec {len(trans)} transactions", "success")
    return redirect(url_for("archives.archives"))
...
def archives():
    ...
    mois_dispo = [{'annee': y, 'mois': m} for y, m in sorted({(t.dateTransaction.year, t.dateTransaction.month) for t in Transaction.query.all()}, reverse=True)
...
def voir_archive(id):
    ...
    return render_template("archive-detail.html", archive=archive, donnees=json.loads(archive.donnees_json or '{}'), nom_mois=mois_noms[archive.mois])
