...
def seed_6_mois():
    ...
    transactions_templates = [
        # Revenus
        {'titre': 'Salaire', 'montant': 2500, 'categorie': 'Salaire', 'type': 'revenu'},
        ...
        {'titre': 'Électricité', 'montant': -65, 'categorie': 'Logement', 'fixe': True, 'jour': 10},
    ...
    for mois_offset in range(6, 0, -1):
        date_mois = now - timedelta(days=mois_offset * 30)
        annee = date_mois.year
        mois = date_mois.month

        print(f"\n[SEED] Mois {mois}/{annee}...")
    ...
    for obj_data in objectifs_data:
        ...
        if not existing:
            ...
            db.session.add(obj)
            print(f"  ✓ Objectif créé: {obj_data['description']}")

    db.session.commit()

    # 4. Archives pour les mois passés
    ...
    for mois_offset in range(6, 1, -1):
        ...
        donnees = {
            'transactions': [
                {'titre': t.titre, 'montant': float(t.montant), 'date': t.dateTransaction.strftime('%Y-%m-%d'),
                 'categorie': t.categorie.nom if t.categorie else 'Autre'}
                for t in trans_mois
            ],
            'stats': {'nb_transactions': len(trans_mois)}
        }

        archive = ArchiveMensuelle(
            annee=annee, mois=mois,
            total_revenus=revenus, total_depenses=depenses,
            total_epargne=0, solde_final=solde,
            donnees_json=json.dumps(donnees)
        )
        db.session.add(archive)
        print(f"  ✓ Archive créée: {mois}/{annee}")
